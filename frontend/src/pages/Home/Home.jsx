...
const Home = () => {
  ...
  useEffect(() => {
    ...
    let x = -el.scrollWidth / 2; // 👈 bắt đầu từ giữa để loop mượt
    const speed = 0.5;
    let raf;
    let isHover = false;
    ...
    const animate = () => {
      if (!isHover) {
        x += speed; // 👈 chạy sang phải
      }

      const halfWidth = el.scrollWidth / 2;

      if (x >= 0) {
        x = -halfWidth; // 👈 reset ngược lại
      }
  ...
  return (
    <div className="relative z-0 flex flex-col w-full">
      <div className="sticky top-0 z-90 h-0 w-full transition-all duration-[500ms]">
        <div className="absolute top-0 h-24 w-full bg-white/90 backdrop-blur-sm shadow-md">
          <div className="h-full w-full flex items-center justify-between px-8 lg:px-16">
            <div className="h-full w-full flex items-center justify-start p-4 gap-4">
              <img src={HcyuLogo} className="h-full object-contain" />
              <div>Chi đoàn khu phố Đông B</div>
            </div>
            <div className="hidden lg:flex w-full items-center justify-end gap-8">
              {MENU.map((item) => (
                <div key={item.id}>
                  <button onClick={() => handleScrollToId(item.id)}>
                    {item.label}
                  </button>
                </div>
              ))}
      ...
      <div className="flex flex-col relative z-10">
        <div
          id="hero-section"
          className="relative z-0 lg:h-screen h-fit lg:pt-24 pt-40 pb-16 lg:pb-0"
        >
          <div
          ...
          >
            <div className="lg:text-4xl text-xl">Công trình thanh niên</div>
            <div className="lg:text-6xl text-2xl font-bold uppercase bg-linear-to-r bg-gradient-to-r from-blue-500 via-sky-500 to-cyan-400 bg-clip-text text-transparent py-1">
...
text-white transition-all duration-300 from-blue-500 via-sky-500 to-cyan-400 "
        ...
        <div
          id="intro-section"
          className="min-h-screen h-fit w-full pt-30 md:pt-24 flex items-center flex-col lg:flex-row md:px-16 gap-8 md:gap-16 relative z-0 overflow-hidden px-8 md:px-0 bg-linear-to-r from-cyan-200 to-blue-300"
        >
          <div
            className={`absolute inset-x-0 bottom-0 w-full -z-120 transition-all h-full duration-[3000ms] ${
              mounted ? "translate-y-0 opacity-70" : "translate-y-4 opacity-0"
            }`}
          >
            <img src={HomeIntroPic1} className="h-full w-full object-cover" />
          </div>
          <div className="flex-2 flex flex-col gap-6 bg-white/60 p-8 backdrop-blur-sm rounded-xl">
            <div className="text-4xl md:text-5xl font-bold bg-linear-to-r bg-gradient-to-r from-indigo-700 to-blue-500 bg-clip-text text-transparent">
              Kết nối thanh niên
            </div>
        ...
        <div
          id="teams-section"
          className="min-h-screen md:h-screen h-fit pt-30 pb-6 flex flex-col items-center justify-center gap-12 md:gap-0 relative"
        >
          {" "}
          ...
          <div className="text-2xl md:text-4xl font-bold">
            Ban chấp hành nhiệm kỳ 2025-2026
          </div>
          <div className="flex flex-col md:flex-row h-full w-full gap-12 items-center justify-center px-12 md:px-16">
            {MEMBERS.map((item) => (
              <div className="w-full flex h-120 relative z-0 overflow-hidden hover:scale-108 scale-100 transition-all duration-[800ms] rounded-xl group">
                ...
                <div className="relative z-10 flex flex-col text-white items-center hover:pt-24 py-12 gap-6 w-full group transition-all duration-[800ms]">
                  <div className="h-48 w-48 rounded-full translate-y-1/2 group-hover:translate-y-0 transition-all duration-[800ms]">
                    <img
                      src={item.avatar}
                      className={`h-full w-full object-cover rounded-full shadow-cyan-100 group-hover:shadow-lg transition-all duration-[800ms] ${item?.fix}`}
                    />
                  </div>
                  <div className="items-center justify-center text-center flex flex-col opacity-0 group-hover:opacity-100 transition-all duration-[800ms] gap-2">
        ...
        >
          <div className="flex flex-col h-full w-full">
            <div className="h-64 flex-shrink-0 overflow-hidden relative z-0">
              <div
  ...
  text-white flex items-end group-hover:min-h-12 group-hover:p-3 min-h-0 h-0 overflow-hidden transition-all duration-500 rounded-b-lg"
                    >
                      Chương trình Tuổi trẻ xung kích
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div className="h-30 justify-center flex items-center">
              <div className="uppercase text-4xl md:text-6xl font-bold bg-linear-to-r bg-gradient-to-r from-blue-500 via-sky-500 to-cyan-400 bg-clip-text text-transparent py-2">
                Hành trình tuổi trẻ
              </div>
            </div>
            <div className="h-64 flex-shrink-0 overflow-hidden relative z-0">
  ...
  text-white flex items-end group-hover:min-h-12 group-hover:p-3 min-h-0 h-0 overflow-hidden transition-all duration-500 rounded-b-lg"
        ...
        <div
          id="contact-section"
          className="min-h-screen h-screen flex flex-row items-center justify-center border relative z-0 px-16 gap-16"
        >
          ...
          <div className="w-full">
            <img src={HomeContactPic2} className="rounded-lg shadow-lg" />
          </div>
          <a  href="https://www.facebook.com/profile.php?id=100093262587854" target="blank" className="w-full flex flex-row gap-4 items-center border justify-center bg-white py-4 rounded-lg">
            <img src={FacebookLogo} className="h-20 w-20" />
            <div>
              <span
               
                className="text-4xl font-bold  bg-gradient-to-r from-blue-500 via-sky-500 to-cyan-500 bg-clip-text text-transparent"
              >
                Tuổi trẻ Đông B
              </span>
            </div>
